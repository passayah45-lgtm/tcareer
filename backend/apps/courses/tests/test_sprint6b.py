...
from apps.courses.models import Lesson, LessonType
...
def course_with_questions(db, instructor):
    ...
    questions = [
        QuizQuestion.objects.create(
            course=course, question_text='Question ' + str(i) + '?',
            options=['A','B','C','D'], correct_index=0, position=i*10
        ) for i in range(1, 4)
    ]
...
class TestLessonReorderEndpoint:
    ...
    def test_wrong_instructor_forbidden(self, other_instructor_client, course_with_lessons):
        course, lessons = course_with_lessons
        payload = {'lessons': [{'id': str(l.id), 'position': i*10} for i, l in enumerate(lessons)]}
...
class TestLessonInlineUpdateEndpoint:
    ...
    def test_update_content(self, instructor_client, course_with_lessons):
        course, lessons = course_with_lessons
        r = instructor_client.patch(self.url(lessons[0].id), {'content': 'New content here'}, format='json')
        assert r.status_code == status.HTTP_200_OK
        lessons[0].refresh_from_db()
    ...
    def test_update_free_preview(self, instructor_client, course_with_lessons):
        course, lessons = course_with_lessons
        r = instructor_client.patch(self.url(lessons[0].id), {'is_free_preview': True}, format='json')
        assert r.status_code == status.HTTP_200_OK
        lessons[0].refresh_from_db()
    ...
    def test_nonexistent_lesson_404(self, instructor_client):
        r = instructor_client.patch('/api/v1/courses/lessons/' + str(uuid.uuid4()) + '/', {'title': 'X'}, format='json')
        assert r.status_code == status.HTTP_404_NOT_FOUND

    def test_publish_text_lesson(self, instructor_client, course_with_lessons):
        course, lessons = course_with_lessons
        text = LessonFactory(course=course, lesson_type=LessonType.TEXT, is_published=False, position=50)
...
class TestQuizBulkCreateEndpoint:
    ...
    def q(self, text='What is Python?'):
        if len(text) < 5:
            text = text + ' here'
        return {'question_text': text, 'options': ['A language','A snake','A tool','An IDE'], 'correct_index': 0}

    def test_bulk_create_success(self, instructor_client, course):
        r = instructor_client.post(self.url(course.id), {'questions': [self.q('Q1?'), self.q('Q2?')], 'replace': False}, format='json')
    ...
    def test_persists_to_db(self, instructor_client, course):
        instructor_client.post(self.url(course.id), {'questions': [self.q('Q1?'), self.q('Q2?')], 'replace': False}, format='json')
    ...
    def test_append_by_default(self, instructor_client, course_with_questions):
        ...
        instructor_client.post(self.url(course.id), {'questions': [self.q('New?')], 'replace': False}, format='json')
    ...
    def test_replace_deletes_existing(self, instructor_client, course_with_questions):
        course, _ = course_with_questions
        r = instructor_client.post(self.url(course.id), {'questions': [self.q('Only?')], 'replace': True}, format='json')
        assert r.status_code == status.HTTP_201_CREATED
        data = r.json()['data']
        assert data['total'] == 1
    ...
    def test_wrong_instructor_forbidden(self, other_instructor_client, course):
        r = other_instructor_client.post(self.url(course.id), {'questions': [self.q()], 'replace': False}, format='json')
        assert r.status_code == status.HTTP_403_FORBIDDEN

    def test_student_forbidden(self, student_client, course):
        r = student_client.post(self.url(course.id), {'questions': [self.q()], 'replace': False}, format='json')
        assert r.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_empty_questions_rejected(self, instructor_client, course):
        r = instructor_client.post(self.url(course.id), {'questions': [], 'replace': False}, format='json')
        assert r.status_code == status.HTTP_400_BAD_REQUEST

    def test_too_many_questions_rejected(self, instructor_client, course):
        r = instructor_client.post(self.url(course.id), {'questions': [self.q('Q' + str(i) + '?') for i in range(51)], 'replace': False}, format='json')
        assert r.status_code == status.HTTP_400_BAD_REQUEST

    def test_invalid_correct_index(self, instructor_client, course):
        r = instructor_client.post(self.url(course.id), {'questions': [{'question_text': 'Which?', 'options': ['A','B','C','D'], 'correct_index': 5}], 'replace': False}, format='json')
        assert r.status_code == status.HTTP_400_BAD_REQUEST

    def test_correct_index_beyond_options(self, instructor_client, course):
        r = instructor_client.post(self.url(course.id), {'questions': [{'question_text': 'Which?', 'options': ['A','B'], 'correct_index': 3}], 'replace': False}, format='json')
        assert r.status_code == status.HTTP_400_BAD_REQUEST

    def test_single_option_rejected(self, instructor_client, course):
        r = instructor_client.post(self.url(course.id), {'questions': [{'question_text': 'Which?', 'options': ['Only'], 'correct_index': 0}], 'replace': False}, format='json')
        assert r.status_code == status.HTTP_400_BAD_REQUEST

    def test_short_question_text_rejected(self, instructor_client, course):
        r = instructor_client.post(self.url(course.id), {'questions': [{'question_text': 'OK', 'options': ['A','B','C','D'], 'correct_index': 0}], 'replace': False}, format='json')
        assert r.status_code == status.HTTP_400_BAD_REQUEST

    def test_positions_auto_assigned(self, instructor_client, course):
        r = instructor_client.post(self.url(course.id), {'questions': [self.q('Q1?'), self.q('Q2?'), self.q('Q3?')], 'replace': False}, format='json')
        assert r.status_code == status.HTTP_201_CREATED
...
class TestQuizQuestionReorderEndpoint:
    ...
    def test_wrong_instructor_forbidden(self, other_instructor_client, course_with_questions):
        course, questions = course_with_questions
        r = other_instructor_client.post(self.url(course.id), {'questions': [{'id': str(questions[0].id), 'position': 10}]}, format='json')
        assert r.status_code == status.HTTP_403_FORBIDDEN

    def test_unknown_question_id(self, instructor_client, course_with_questions):
        course, _ = course_with_questions
        r = instructor_client.post(self.url(course.id), {'questions': [{'id': str(uuid.uuid4()), 'position': 10}]}, format='json')
        assert r.status_code == status.HTTP_400_BAD_REQUEST
